/**
 * app_bird_game.c
 * Flappy Bird game — pipe-based side-scroller with collision detection and scoring.
 */
 ...
 */
static app_bird_game_state_t app_bird_game_state = GAME_READY;       // 当前游戏状态：准备、运行或结束
static bird_app_bird_game_object_t bird;                    // 小鸟对象，包含位置、大小和速度信息
...
static void draw_pipe(bird_app_bird_game_object_t *pipe);
...
static void app_bird_game_handle_input(void);
static void app_bird_game_update(void);
static void app_bird_game_render(void);
static bool app_bird_game_check_collision(void);
static void app_bird_game_reset(void);
...
void app_bird_game_init(void)
{
    ...
    app_bird_game_state = GAME_READY;       // 设置游戏状态为准备状态
    score = 0;                     // 重置分数
...
{
    app_bird_game_init();  // 初始化游戏
    
    // 游戏主循环
    while (1) {
        app_bird_game_handle_input();  // 处理游戏输入
        app_bird_game_update();        // 更新游戏状态
        app_bird_game_render();        // 渲染游戏画面
        
        // 检查是否有退出请求
        if (app_bird_game_should_exit()) {
            OLED_Clear();    // 清除屏幕
...
{
    app_bird_game_handle_input();
    app_bird_game_update();
    app_bird_game_render();
}
...
static void app_bird_game_handle_input(void)
{
    ...
    if (jump_requested) {
        ...
        // 根据当前游戏状态执行不同操作
        switch (app_bird_game_state) {
            case GAME_READY:  // 游戏准备状态
                app_bird_game_state = GAME_RUNNING;        // 切换到运行状态
...
{
    // 仅在游戏运行状态下更新
    if (app_bird_game_state != GAME_RUNNING) return;
    ...
    if (bird.y < BIRD_HEIGHT / 2) {  // 防止小鸟飞出屏幕顶部
        ...
        app_bird_game_state = GAME_OVER;     // 设置游戏结束状态
    ...
    // 检查小鸟是否与管道发生碰撞
    if (app_bird_game_check_collision()) {
        app_bird_game_state = GAME_OVER;     // 设置游戏结束状态
    }
}
...
{
    // 游戏准备状态，只在初始帧绘制
    if (app_bird_game_state == GAME_READY) {
    ...
    // 游戏结束画面（在运行画面上叠加）
    if (app_bird_game_state == GAME_OVER) {
 ...
 * 重置游戏
 * 
 * 函数调用app_bird_game_init()重新初始化游戏状态
 * 提供一个简单的接口供外部调用
 * 
 * 内部函数 - 仅在模块内部调用
 */
